-- ============================================================================
-- LAUNCH OFFER: first 1000 orders ride free + ৳1000+ always free (2026-09-11)
-- ============================================================================
-- SUPERSEDES 202609100003's free-delivery rule — self-contained: run ONLY
-- this one (safe to re-run). Never apply 202609100001/202609100002.
--
-- Owner decision, 2026-09-11: put the launch offer back up ON TOP of the
-- per-customer promo. Ways to ride free now:
--   1. LAUNCH OFFER — store-wide first 1000 orders: free in ANY zone
--      ("প্রথম 1000 অর্ডারে ডেলিভারি ফ্রি", live X/1000 counter via /api/promo)
--   2. ৳1000+ subtotal → always free, any zone
--   3. Per-customer first 10 orders (normalized phone, cancelled excluded)
--      — still Zone A only
--   4. Store pickup + free-delivery coupons (as before)
--
-- Rules kept: district → upazila → para form, scheduled/evening slots,
-- express, night/rain surcharge, distance/weight surcharge, rider tip,
-- store pickup — all server-priced.
-- ============================================================================
...
begin
  ...
  if v_is_pickup then
    ...
    -- LAUNCH OFFER count: every order ever placed (any status), store-wide.
    select count(*) into v_global_orders from orders;
    if v_global_orders < 1000
       or v_subtotal >= 100000
       or (v_zone.id = 'z1' and v_user_orders < 10) then
      v_charge := 0;
      v_sur_night := 0; v_sur_rain := 0; v_sur_dist := 0; v_sur_express := 0; v_sur_weight := 0;
    else
  ...
  values (
    ...
    'Placed via checkout — ' || v_district || ' / ' || v_upazila || ' / '
      ...
      || ' | launch_free=' || (v_global_orders < 1000 or v_subtotal >= 100000)::text
