-- ============================================================================
-- P1 #8 (2026-09-14): bKash / Nagad payments WITHOUT a merchant account.
--
-- The shop takes money into its OWN personal bKash/Nagad wallet — no merchant
-- portal, no API key, no settlement. The customer sends the exact total at
-- checkout and enters the TRXID; the order lands as
-- payment_status='pending_verification'. The shop checks the TRXID against
-- its wallet and verifies (or rejects) it from the admin order page. Only a
-- verified wallet order may start fulfilment — ps_advance_order gates the
-- fulfilment statuses, and the rider queue can never see an unverified one.
-- COD is untouched and stays the default.
--
-- The wallet numbers live in site_settings['ops'].wallets (admin → Settings),
-- exactly like every other shop lever: a method with no configured number is
