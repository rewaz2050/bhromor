/**
 * POST   /api/stock-watch — "call me when this is back in stock" (P2 #2)
 * DELETE /api/stock-watch — stop waiting
 *
 * The sibling of /api/price-watch. One row per (product, phone). The promise
 * is a call from the shop, not a text from a robot — there is no SMS/email
 * sender in this stack, and the shop already confirms orders by phone.
 * Rate-limited per IP; the number is normalized + format-checked here.
 */
...
import { createStockWatch, deleteStockWatch } from "@/lib/db/growth";
...
const guard = (ip: string) => {
  const bucket = checkRateLimit(`stock-watch:${ip}`, LIMIT, WINDOW_MS);
...
export async function POST(request: Request) {
  ...
  try {
    await createStockWatch(db, checked.value);
    return apiJson({ waiting: true as const }, 201);
  } catch (err) {
...
export async function DELETE(request: Request) {
  ...
  try {
    await deleteStockWatch(db, checked.value);
    return apiJson({ waiting: false as const });
  } catch {
