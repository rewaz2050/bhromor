...
/**
 * Push one milestone to every device the shopper registered on this phone.
 *
 ...
 * — the callers ignore it, but it makes the behaviour testable and lets a
 * future "why did nobody get it" question be answered from a log line.
 */
export const pushOrderMilestone = async (
  ...
  try {
    ...
    if (!phone || !orderNo || !kind) return 0;
    if (!ensureVapid()) return 0;

    const subs = await listForPhone(db, phone);
        ...
        try {
          ...
          const status =
            typeof err === "object" && err !== null && "statusCode" in err
              ...
              : 0;
          // 404/410 = the browser is gone (reinstall, cleared data) → prune.
