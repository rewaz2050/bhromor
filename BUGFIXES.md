...
| 133 | **Homepage was a poster, not a shelf (Batch J — storefront discovery, 2026-09-19)** — the owner's report: shoppers land, see a full-screen photo and one button, and "wander" to find anything. Code confirmed it: the hero filled the whole phone viewport (`100svh − 4.5rem`), the only product section rendered `featured`-starred pieces (and the star never saved until #132, so live showed **zero products**), and there was no best-seller / new-arrival path even though `/shop` had both sorts and `v_product_sales` was live. Rebuilt around Baymard's three product-finding paths (categories, curated, search) — (a) **compact hero** (≈58svh on phones, capped 38rem on desktop) with **Browse chips** inside it: every category with pieces + New arrivals + Best sellers (only when ≥2 real sellers) + All pieces, plus a "The whole shelf ↓" anchor. (b) **Best sellers rail** ranked by real `unitsSold` (in stock only, hidden until the shop has 2 real sellers — no dressed-up guesses) and **New arrivals rail** (`isNew` first, then newest catalog order); headings are links, "See all …" labels carry the full scope; `/shop?sort=best|newest` now pre-sorts the browser (new `lib/shop-sort.ts`, shared by the server page and the client). (c) **The whole shelf**: one block per active category with pieces (in-stock first, drafts/archived never), 8 per block on the homepage, then a scoped "See all N in <category>" button; a saved zone no shop serves says so with a one-tap "Show all zones" reset instead of "being stocked". (d) **Product page tail** = "More in <category>": siblings from the same category first (in stock first, never the piece itself or a complete-the-look item), featured pieces from elsewhere only top the row up, with a scoped See-all link — replaces the mixed "You may also like"; breadcrumb shows the category's real name instead of its id. Pure helpers in `lib/home-shelves.ts`; Bangla + English copy under `home.*`. Tests: homepage journey rewritten (10), +9 shelf helper, +1 sort deep-link (140 files / 960 green). |

### Owner report (2026-09-23) — *"Notification on korte partesi na"* (Nothing CMF Phone 2 Pro)

| # | Fix |
|---|-----|
| 134 | **Phone notifications could not be switched on, and two Android-only defects meant they would not have shown on the phone anyway** — reported from a Nothing CMF Phone 2 Pro (Android 15), where the setup card offered one button and one sentence for five different failures, so "no server keys", "opened inside WhatsApp", "permission was blocked months ago" and "this browser was never registered" all looked the same. (a) *Untapped permission prompt poisoned the browser*: `use-notifications.ts` called `Notification.requestPermission()` on mount, which Chrome penalises by **auto-denying without a prompt** — and `denied` is sticky, so the owner's later tap on "Phone notification ON" could only ever answer "browser ta permission dey nai". Permission is now asked from exactly one place, by tap (the setup card; the orders banner links there instead of half-subscribing). (b) *`new Notification()` is an illegal constructor on every mobile Chrome* ("Use ServiceWorkerRegistration.showNotification() instead"), and the old code swallowed that TypeError — nothing appeared on the phone while the desktop thought it had alerted; in-panel alerts now go through the service worker (`showPanelNotice()`), falling back to the constructor only where it exists. (c) *The beep was silent on Android*: an `AudioContext` created before any gesture starts `suspended` (autoplay policy), so the oscillator played into a dead graph — it is now created/resumed on the first tap and resumed in `playBeep()`. (d) *No honest diagnosis*: `GET /api/admin/push` reports `tableReady` (the head-count error was ignored, so a missing `push_subscriptions` table read as "0 devices, all good"), `POST` answers 503 naming `supabase/migrations/202609210001_push_subscriptions.sql` instead of a generic 422, `pushSaveFailureReason()` maps `42P01`/`PGRST205`/schema-cache errors to "table missing", and `/api/health` gains `pushConfigured`/`pushTableReady` + `counts.push_subscriptions` with Banglish `nextSteps`. (e) *The card is now a live checklist* (`PushSetup` + new `src/lib/push-client.ts`): Server key · Database table · Ei browser (in-app WebViews named out loud, http blocked) · Browser permission (with hand-set recovery steps for a sticky denial, incl. Nothing OS: Chrome's own notification permission + battery "Unrestricted") · Ei phone ta (a live subscription in **this** browser, not the server's device count from someone else's laptop). Opening it with permission already granted re-saves the endpoint silently (self-heal for a rotated/pruned subscription, incl. re-subscribing when the VAPID key changed instead of throwing InvalidStateError). `sw.js` gains `skipWaiting`/`clients.claim`; `.env.example` documents the VAPID pair that the whole feature silently depended on. Tests: `push-client.test.ts` (13), `push-setup.test.tsx` (7), `use-notifications.test.tsx` (2), admin push route (+2), health route (+1 push signals). Full suite 172 files / 1111 green; `tsc` 0, `eslint` 0, `next build` clean. |

---

## New files

- `src/lib/home-shelves.ts` — homepage shelves: category blocks, best sellers (real sales), new arrivals, "more in category"
- `src/lib/shop-sort.ts` — shop sort keys + `?sort=` parser shared by server page and client browser
- `src/components/home/product-rail.tsx`, `src/components/home/category-shelf.tsx`, `src/components/product/more-in-category.tsx`
